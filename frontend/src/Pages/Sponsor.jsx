...
const SponsorCard = ({ name, logo, logo1, logo2, size = 'md', role, isTitle }) => {
  ...
  return (
    <div className={`flex items-center justify-center`}>
      ...
      >
        {logo1 && logo2 ? (
  <div className={`flex flex-col sm:flex-row items-center justify-center gap-4 bg-white shadow-lg  w-full h-full `}>
    <img
      src={logo1}
      alt={`${name} logo 1`}
      className={`
        object-contain max-h-full max-w-full  sm:w-1/2 w-3/4
        ${logo1?.includes('tektronix.svg') ? 'filter invert' : ''}
      `}
    />
    <img
      src={logo2}
    ...
    <div ref={containerRef} className="min-h-screen flex items-center justify-center p-6 relative">
      ...
      <div className="max-w-7xl mx-auto w-full">
        ...
        <section className="mb-16 sponsors-section px-4">
          ...
          <div className="flex flex-wrap sm:grid-cols-3 md:grid-cols-6 gap-8 p-4">
